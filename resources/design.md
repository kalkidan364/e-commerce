...
- **Frontend**: React 18+ with TypeScript, Redux for state management, React Router for navigation
- **Backend**: Node.js with Express.js, TypeScript for type safety
- **Database**: PostgreSQL 14+ for relational data, Redis 7+ for caching and sessions
...
4. **API Server**: 
   ...
   - Queries PostgreSQL for data or checks Redis cache
...
interface ProductService {
  listProducts(filters: ProductFilters, pagination: Pagination): Promise<ProductPage>
...
interface CartService {
  ...
  addItem(userId: string, productId: string, variantId: string, quantity: number): Promise<Cart>
  ...
  validateCart(userId: string): Promise<CartValidation>
  calculateTotal(cart: Cart): Promise<CartTotal>
}
```
...
interface CheckoutService {
  initiateCheckout(userId: string): Promise<CheckoutSession>
  applyDiscount(sessionId: string, code: string): Promise<CheckoutSession>
  calculateShipping(sessionId: string, addressId: string, method: ShippingMethod): Promise<ShippingCost>
  processPayment(sessionId: string, paymentMethod: PaymentMethod): Promise<Order>
  createStockReservations(items: CartItem[]): Promise<Reservation[]>
  releaseReservations(reservationIds: string[]): Promise<void>
}
```
...
interface OrderService {
  ...
  getUserOrders(userId: string, pagination: Pagination): Promise<OrderPage>
  cancelOrder(id: string, userId: string): Promise<Order>
  reorder(orderId: string, userId: string): Promise<Cart>
  updateOrderStatus(id: string, status: OrderStatus): Promise<Order>
  updateTracking(id: string, tracking: TrackingInfo): Promise<Order>
  processRefund(id: string): Promise<Order>
}
```
...
interface InventoryService {
  ...
  reserveStock(items: CartItem[], ttl: number): Promise<Reservation[]>
  releaseReservation(reservationId: string): Promise<void>
  deductStock(items: OrderItem[]): Promise<void>
  restoreStock(items: OrderItem[]): Promise<void>
...
interface PaymentService {
  ...
  confirmPayment(intentId: string, paymentMethod: PaymentMethod): Promise<PaymentResult>
  processRefund(chargeId: string, amount: number): Promise<Refund>
...
interface NotificationService {
  ...
  sendLowStockAlert(product: Product, quantity: number): Promise<void>
}
```
...
interface ProductVariant {
  ...
  priceAdjustment: number       // In cents, added to basePrice
...
interface Category {
  ...
  parentId: string | null       // Self-referential for hierarchy
  description: string
  sortOrder: number
  isActive: boolean
}
...
interface OrderItem {
  ...
  quantity: number
  pricePerUnit: number         // In cents, snapshot
  subtotal: number             // In cents
}
```
...
interface DiscountCode {
  ...
  minimumOrderValue: number | null // In cents
